                                  ...
                                  child: ElevatedButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (usernameController.text.trim() != null &&
                                          ...
                                          //when email exist, then check if it is his own
                                          if (mapUserinformations!.isNotEmpty) {
                                            print('email is already existing');

                                            //check if it is his own email
                                            if (selectedRows[0]['email'] == mapUserinformations!['email']){
                                              print('own email');

                                              mapUsernameExist = await authproviderInstance.UsernameExist(usernameController.text.trim());
                                              //if username already exist, then print error otherwise it is free
                                              if(mapUsernameExist!.isEmpty || mapUsernameExist!['username'] == selectedRows[0]['username']){
                                                print('username is free oder eigene');
